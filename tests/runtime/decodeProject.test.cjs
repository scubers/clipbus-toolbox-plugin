...
test("manifest declares the toolbox plugin identity and the decode feature", () => {
  ...
  assert.equal(manifest.attachmentRenderers[0].attachmentType, "plugin.clipbus.toolbox.decode.preview");
  ...
  assert.deepEqual(manifest.detectors[0].attachmentTypes, ["plugin.clipbus.toolbox.decode.preview"]);
...
test("package declares toolbox identity and verification scripts", () => {
  ...
  assert.equal(packageJSON.name, "@clipbus/toolbox-plugin");
...
test("decode UI uses SDK APIs instead of removed bridge APIs", () => {
  ...
  assert.ok(source.includes("clipbus.attachmentRenderer.onHostInvoke.on"));
  assert.ok(source.includes("clipbus.attachmentRenderer.setButtons"));
  // Height is driven by the SDK autoFit helper from @clipbus/plugin-sdk/dom
  // (replaces the hand-rolled ResizeObserver/MutationObserver + manual
  // clipbus.window.setHeight loop used before the template migration). autoFit
  // calls window.setHeight internally.
  assert.ok(source.includes("@clipbus/plugin-sdk/dom"));
  assert.ok(source.includes("autoFit("));
  assert.ok(!source.includes("clipbus.window.autoFit"));
  assert.ok(!source.includes("clipbus.ready"));
  assert.ok(!source.includes("clipbus.item.setAttachments"));
  assert.ok(!source.includes("window.webkit.messageHandlers"));
  assert.ok(!source.includes("addEventListener(\"clipbus-plugin-"));
});
