  ...
  <main class="shell expanded-shell">
    <section v-if="payload" class="expanded-panel">
      ...
      <div
        ...
        :style="{ backgroundColor: accentColor || 'var(--pasty-accent, #2563EB)' }"
        aria-hidden="true"
      />
...
let disconnectAutoFit = null;
let disconnectTheme = null;
...
function readBootstrapPayload() {
  ...
  return safeParsePayload(attachment?.payloadJson);
}

function handleAttachmentUpdate(attachment) {
  const next = safeParsePayload(attachment?.payloadJson);
  if (next) {
    payload.value = next;
  }
}

function handleRendererAction(detail) {
  // Native host action buttons (defined by runtime's resolveAttachment) dispatch
  // this event so the WebView can react in parallel to the runtime's
  // invokeOperation. We use it to toggle the debug section from the host
  // "Toggle" action, matching the in-WebView Show/Hide Debug button.
...
async function attachAutoFitIfReady() {
  ...
  await nextTick();
  const target = document.querySelector(".expanded-panel");
  if (!target) {
    return;
  }
  disconnectAutoFit = pasty.window.autoFit({ min: 120, max: 480, target });
}

onMounted(async () => {
  payload.value = readBootstrapPayload();

  // Subscribe to attachment updates via SDK
  ...
  unsubHostInvoke = pasty.item.attachment.onHostInvoke(handleRendererAction);
  ...
  // Initial accent: try pasty.theme.refresh() once. In local preview /
  // hosts without pastyPluginHostSync wired up this returns null, so we fall
  // back to reading the CSS custom property directly.
  try {
    const snapshot = await pasty.theme.refresh();
    if (snapshot?.tokens) {
      accentColor.value = snapshot.tokens["--pasty-accent"] ?? accentColor.value;
    }
  } catch {
    // ignore — fallthrough to fallback below
  }
  if (!accentColor.value && typeof window !== "undefined") {
    const computed = getComputedStyle(document.documentElement).getPropertyValue("--pasty-accent").trim();
    if (computed) {
      accentColor.value = computed;
    }
  }

  // Live updates: pasty.theme.on() fires whenever the host appearance
  // changes (light/dark). CSS variables already update automatically, but we
  // need JS access for the inline-styled accent strip.
  disconnectTheme = pasty.theme.on(({ tokens }) => {
    accentColor.value = tokens["--pasty-accent"] ?? accentColor.value;
  });
});

onUnmounted(() => {
  if (typeof unsubAttachment === "function") {
    unsubAttachment();
    unsubAttachment = null;
  }
  ...
  if (typeof disconnectTheme === "function") {
    disconnectTheme();
    disconnectTheme = null;
  }
});

const extendedInfo = computed(() => payload.value?.extended ?? null);
