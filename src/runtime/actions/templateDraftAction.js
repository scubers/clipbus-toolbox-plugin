...
function resolveInitialTitle(item, content) {
  const display = buildItemDisplay(item, content);
...
async function readConfiguredLabel(ctx) {
  const value = await ctx?.host?.settings?.get?.("plugin.template.full.label");
  const label = typeof value === "string" ? value.trim() : "";
  return label || "Template Draft Action";
}
  ...
  return {
    async resolveSession(input, ctx) {
      ...
      return {
        ...
        defaultButtonID: "apply-metadata",
        initialDraft: {
          title: resolveInitialTitle(item, content),
