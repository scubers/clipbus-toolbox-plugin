...
关键 SDK 事实（核对 `@pasty/plugin-sdk@0.3.0` 的 `API.md` / `README.md` / 生成 `.d.ts`，均为实际编译依赖的 node_modules 版本）：

- draft action 完整支持：`manifest.actions[].lifecycle:"draft"` + 必填 `uiEntry`；`resolveSession(input,ctx)`（可选）播种 `initialDraft` + seed 按钮；UI 自管表单，最终 `pasty.action.complete({result})` 提交。**没有** `pasty.action.draft.update()`，draft 只读。
- **`pasty.asset.currentItemImageUrl()`（0.3.0 新增，UI 侧，context 任意）→ `{url?}`**：返回不透明 `pasty-asset://` 令牌 URL，`<img src>` 直接渲染当前 item 原图，真实路径不进 JS（沙箱 block `file://`）。**这是 WebView 显示原图的指定机制**——无需 base64、无需 runtime 往返。
- image 结果的 `imageTempPath` **只能** runtime 侧 `host.action.allocateImageTempPath({formatHint})` 分配（UI 端无此能力）；runtime 写文件后把路径回传 UI，UI 用 `pasty.action.complete({result:{resultKind:"image",imageTempPath,imageFormatHint}})` 提交。
- runtime 侧 `host.item.materializeImagePath()→{path}` 把原图复制到临时文件供 sharp 读取（invocation 结束宿主自动清理；同一 invocation 幂等）。
- **没有 `pasty.ready()`**（0.3.0 移除）：模块加载即注册 `.on()`，`.current()` 取快照并用 `?.`/`??` 兜底。`autoFit` 在 `@pasty/plugin-sdk/dom`。
...
> 旁注：仓库内嵌 `sdk/` 是过时副本（无 `asset`、含已废弃 `pasty.ready`），**不参与构建**（`tsconfig` 无 paths 别名、`vite` 无 alias，源码 import `@pasty/plugin-sdk` 解析到 node_modules@0.3.0）。实现时以 node_modules 的类型为准，勿被 `sdk/` 误导。
...
Draft UI（Vue WebView）：
   const { url } = await pasty.asset.currentItemImageUrl(); img.src = url   ← 原图全分辨率直显
   裁剪框覆盖层（自研）+ 质量滑块 + 实时读数（宽 × 高 px · 宽高比）
        │  用户点「应用」(pasty.action.onHostInvoke → buttonID==="apply")
        ▼
   pasty.runtime.invoke("image-edit/process-image", { quality, crop:{x,y,width,height}(原图像素) })
        ...
        ▼
   pasty.action.complete({ result:{resultKind:"image", imageTempPath, imageFormatHint}, userMessage:"已裁剪并压缩" })
        ▼
宿主原生 UI 展示结果图（原图不动）
```
...
- `feature.ts` / `process.ts`（esbuild 打进 `dist/plugin.cjs`）只 import `@pasty/plugin-sdk/runtime`、`sharp`、`contracts.ts`。
...
用纯字符串 key + 共享 interface（不引 `defineMessage`），避免 isomorphic 模块跨引 SDK 的 `/ui` 与 `/runtime` 入口。UI 用 `pasty.runtime.invoke<ProcessImageResp>({key:PROCESS_IMAGE, payload})`；runtime 用 `messageHandlers[PROCESS_IMAGE]`（ctx 含 `host`）。
...
- **取图**：`onMounted` → `const {url}=await pasty.asset.currentItemImageUrl(); if(url) previewUrl=url;` 否则显示错误占位。
- **坐标基准**：原图 `W×H` 取自 `pasty.action.draft.current()`（兜底 `img.naturalWidth/Height`）。`<img @load>` 后量 `clientWidth` → 显示比例 `s = clientWidth / origWidth`。裁剪框以**显示坐标**存储；读数与提交时 `÷s` 并 `Math.round` 得原图像素（`cropGeometry.ts` 负责换算）。
...
- **提交**：`pasty.action.onHostInvoke(({buttonID})=> buttonID==="apply" && apply())`。`apply()`：先 `setButtons([{id:"apply",title:"应用",isEnabled:false}])` 防重复 → `await pasty.runtime.invoke<ProcessImageResp>({key:PROCESS_IMAGE,payload:{quality,crop:原图裁剪},timeoutMs:60_000})` → `await pasty.action.complete({result:{resultKind:"image",imageTempPath,imageFormatHint},userMessage:"已裁剪并压缩"})`。失败 → 恢复按钮 + 显示错误文案，不调用 `complete`。
- 样式复用 `shared/base.css` + `pasty.theme` 主题 token；用 `@pasty/plugin-sdk/dom` 的 `autoFit({min,max,target})` 适配面板高度。无 `pasty.ready()`。
...
- **vite workbench（`pnpm dev`，`?view=action`）**：新增 action 预览 scenario，mock `pasty.asset.currentItemImageUrl` 回传内置样图（data URL 或 vite 静态资源），让裁剪框/滑块/读数可手动开发验证；`process-image` / `complete` 在 workbench 中打日志空跑。
- **真机端到端**：仅真实 Pasty 宿主可验，列入下方延迟项。
...
2. `pasty-asset://` 在宿主 WebView CSP 下渲染（宿主自家协议，风险低；但本插件页若加 CSP 需放行 `img-src pasty-asset:`）。
