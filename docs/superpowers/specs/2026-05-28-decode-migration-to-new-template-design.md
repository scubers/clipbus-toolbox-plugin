...
本工程是 Pasty 粘贴板的三方插件 `plugin.pasty.awesome.decode`（Pasty Awesome Decode）——一个纯文本解码插件。模板上游更新后，作者把新模板整体覆盖进仓库：旧的 `decode-renderer` feature、仓库内 `sdk/` 源码、decode 测试被删除，取而代之的是一套新示例 feature（preview-renderer / expanded-renderer / auto-action / capability-gallery）和把 SDK 改为外部 npm 包 `@pasty/plugin-sdk@^0.2.0` 的新构建管线。
...
- `node_modules` 经 `npm install` 已就绪（含 `@pasty/plugin-sdk` 0.2.0）。

## 3. SDK 0.2.0 兼容性结论（关键）

旧 decode 代码本就针对 `@pasty/plugin-sdk/{runtime,ui,dom}` 编写（HEAD 提交 `b34084c` 即"迁移到模板 SDK"）。核对已安装的 0.2.0 类型，旧代码用到的导出**全部仍在**：
...
- UI：`pasty.item.attachment`、`pasty.attachmentRenderer.{setButtons,onHostInvoke}`、`pasty.clipboard.copyText`、`pasty.window.setHeight`、`pasty.settings.get`、`PluginAttachmentPayload` — 均可用。
- `@pasty/plugin-sdk/dom`：`patchConsole`、`patchTextInputState`、`autoFit(options?: { min?, max?, target? }): () => void` — 可用。
...
### 4.2 恢复后修改
- `src/features/decode-renderer/app.vue`：恢复后 (a) 把手写 `ResizeObserver`/`MutationObserver`/`requestAnimationFrame`/`pasty.window.setHeight` 高度同步替换为 `autoFit({ min: 32, max: 480, target: rootEl })`；(b) 把 `!= null` / `== null` 改为严格比较（满足 eslint `eqeqeq`）；(c) 清理因此空出的未使用导入/变量。
...
- `manifest.json`、`src/plugin.ts`：恢复 HEAD 的 decode 版本（`plugin.pasty.awesome.decode`、`permissions:["setAttachment"]`、1 detector `decode-detector`(text) + 1 renderer `decode-renderer`，attachmentType `plugin.pasty.awesome.decode.preview`）。
...
- `src/preview/scenarios/attachmentScenarios.ts`：产出 `DecodePayload` 样本（Base64 / JWT / URL / 时间戳 等若干），`pluginID=plugin.pasty.awesome.decode`、`rendererID=decode-renderer`、`attachmentType=plugin.pasty.awesome.decode.preview`，注入 `__PASTY_PLUGIN_*__` + 派发 `pasty-plugin-*` 事件的机制沿用。
